...
u_char *HPFeedsReadMsg(int sock)
{
  ...
  if (read(sock, &msglen, 4) != 4) {
    SCLog(SC_LOG_ERROR,"alert-hpfeeds: Fatal read()");
    return NULL;
  }

  if ((buffer = malloc(ntohl(msglen))) == NULL) {
    SCLog(SC_LOG_ERROR,"alert-hpfeeds: Fatal malloc()");
    return NULL;
  }
...
void HPFeedsGetError(hpf_msg_t *msg, int sock) 
{
  ...
  if (msg) 
  {
    if ((errmsg = calloc(1, msg->hdr.msglen - sizeof(msg->hdr))) == NULL) {
      SCLog(SC_LOG_ERROR,"alert-hpfeeds: Fatal write()");
      hpf_msg_delete(msg);
      return;
    }
...
void HPFeedsConnect(AlertHPFeedsCtx *config, int reconnect) 
{
  ...
  if ((he = gethostbyname((char *)config->hpfeeds_host)) == NULL) {
    SCLog(SC_LOG_ERROR,"alert-hpfeeds: Fatal gethostbyname()\n");
    return;
  }
  ...
  if ((config->sock = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP)) == -1) {
    SCLog(SC_LOG_ERROR,"alert-hpfeeds: Fatal socket()");
    return;
  }

  if (connect(config->sock, (struct sockaddr *) &host, sizeof(host)) == -1) {
    SCLog(SC_LOG_ERROR,"alert-hpfeeds: Fatal connect()");
    return;
  }
  ...
  {
      SCLog(SC_LOG_ERROR,"alert-hpfeeds: Fatal setsockopt()");
...
void TmModuleAlertHPFeeds(void)
{
#ifndef OS_WIN32
    ...
    OutputRegisterPacketModule(MODULE_NAME, "alert-hpfeeds",
        AlertHPFeedsInitCtx, AlertHPFeedsLogger, AlertHPFeedsCondition);
